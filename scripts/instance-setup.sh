...
echo "4. Deploy application with comprehensive monitoring:"
echo "   # Use the new monitoring deployment script:"
echo "   ./scripts/deploy-monitoring.sh"
echo ""
echo "   # OR deploy manually in correct sequence:"
echo "   kubectl apply -f k8s/base/namespace.yaml"
echo "   kubectl apply -f k8s/base/configmap.yaml"
echo "   kubectl apply -f k8s/base/mongodb-exporter-fixed.yaml"
echo "   kubectl wait --for=condition=ready pod -l app=mongodb -n ecommerce --timeout=300s"
...
echo "6. Get NodePorts: kubectl get services -n ecommerce"
echo
